  ...
  for(i in (1:nrow(SIR.unique))){
    assign(paste("plot", i, sep = ""), 
           epid(eta=SIR.unique$eta[i],alpha=SIR.unique$alpha[i],
                ...
                distancing=SIR.unique$distancing[i],employment=SIR.unique$employment[i],initialinfected = SIR.unique$initialinfected[i], #contactscaling =  SIR.unique$contactscaling[i], 
                gamma8 = SIR.unique$gamma8[i], #gamma9 = SIR.unique$gamma9[i], 
                UR1 = SIR.unique$UR1[i],  # UR2 = SIR.unique$UR2[i],   UR3 = SIR.unique$UR3  ,UR4 = SIR.unique$UR4[i], 
                endphase = SIR.unique$endphase[i], 
...
# print(plot2$E)
  ...
  dataRt=data.frame(Date=plot1$Date)
  for (i in 1:nrow(SIR.unique)) {
    dataRt[,1+i] <- eval(parse( text=paste("plot", i, sep = "") ))$Rt
  }
  datatemp <- dataRt[,2:ncol(dataRt)]
  datatemp2 <- datatemp
  datatemp$median <- apply(  datatemp2 ,1,median,na.rm=TRUE)
  datatemp$lower <- apply(  datatemp2 ,1,quantile,probs=c(0.025),na.rm=TRUE)
  datatemp$upper <- apply(  datatemp2 ,1,quantile,probs=c(0.975),na.rm=TRUE)
  datatemp$lower2 <- apply(  datatemp2 ,1,quantile,probs=c(0.25),na.rm=TRUE)
  datatemp$upper2 <- apply(  datatemp2 ,1,quantile,probs=c(0.75),na.rm=TRUE)
  dataRt$median<- datatemp$median
  dataRt$lower<- datatemp$lower
  dataRt$upper<- datatemp$upper
  dataRt$lower2<- datatemp$lower2
  dataRt$upper2<- datatemp$upper2
  ...
  datanewcases$lower2<- datatemp$lower2
  datanewcases$upper2<- datatemp$upper2
  ...
  dataSEROPREV =data.frame(Date=plot1$Date)
  for (i in 1:nrow(SIR.unique)) {
    dataSEROPREV[,1+i] <- eval(parse( text=paste("plot", i, sep = "") ))$E + eval(parse( text=paste("plot", i, sep = "") ))$A + eval(parse( text=paste("plot", i, sep = "") ))$I + eval(parse( text=paste("plot", i, sep = "") ))$H + eval(parse( text=paste("plot", i, sep = "") ))$C + eval(parse( text=paste("plot", i, sep = "") ))$R  
  }
  datatemp <- dataSEROPREV[,2:ncol(dataSEROPREV)]
  datatemp2 <- datatemp
  datatemp$median <- apply(  datatemp2 ,1,median,na.rm=TRUE)
  datatemp$lower <- apply(  datatemp2 ,1,quantile,probs=c(0.025),na.rm=TRUE)
  datatemp$upper <- apply(  datatemp2 ,1,quantile,probs=c(0.975),na.rm=TRUE)
  dataSEROPREV$median<- datatemp$median
  dataSEROPREV$lower<- datatemp$lower
  dataSEROPREV$upper<- datatemp$upper
  
  summm <- sum(AGEPOP)
  dataSEROPREVsum=data.frame("Date"=as.Date(plot1$Date, format = "%d/%m/%Y"),
                     ...
                     "Median"=  round(dataSEROPREV$median/summm,5)*100,
                     "Lower" = round(dataSEROPREV$lower/summm,5)*100,
                     "Upper" = round(dataSEROPREV$upper/summm,5)*100
                     )
  ...
  datadying =data.frame(Date=plot1$Date)
  for (i in 1:nrow(SIR.unique)) {
    datadying[,1+i] <- eval(parse( text=paste("plot", i, sep = "") ))$dying.in.nonIC + eval(parse( text=paste("plot", i, sep = "") ))$dying.in.IC
  }
  datatemp <- datadying[,2:ncol(datadying)]
  datatemp2 <- datatemp
  datatemp$median <- apply(  datatemp2 ,1,median,na.rm=TRUE)
  datatemp$lower <- apply(  datatemp2 ,1,quantile,probs=c(0.025),na.rm=TRUE)
  datatemp$upper <- apply(  datatemp2 ,1,quantile,probs=c(0.975),na.rm=TRUE)
  datatemp$lower2 <- apply(  datatemp2 ,1,quantile,probs=c(0.25),na.rm=TRUE)
  datatemp$upper2 <- apply(  datatemp2 ,1,quantile,probs=c(0.75),na.rm=TRUE)
  datadying$median<- datatemp$median
  datadying$lower<- datatemp$lower
  datadying$upper<- datatemp$upper
  datadying$lower2<- datatemp$lower2
  datadying$upper2<- datatemp$upper2
  ...
  plotdet =list(xlab(label=""), 
                ...
                geom_vline(xintercept=COVIDBYDAY[time2,]$Date, colour=wes_palette("Zissou1",n=5)[5]) ,
             #   geom_vline(xintercept=COVIDBYDAY[time3,]$Date, colour=wes_palette("Zissou1",n=5)[5]) ,
              #  geom_vline(xintercept=lockdownrel, colour=wes_palette("Zissou1",n=5)[5],linetype = "dashed") ,
                theme(plot.title = element_text(hjust = 0.5,size=13)), #,
               scale_x_date(limits = as.Date(c(COVIDBYDAY$Date[1],max(COVIDBYDAY$Date[completeat], COVIDBYDAY[1,]$Date+lengthsim) )))
  )
  #  print(datanew.cases.entering.hospital.cumsum)
  PRt<- ggplot(data= dataRt, aes(x=Date))+
 #   annotate(geom = "rect", xmin=as.Date(last(COVIDBYDAY$Date),"%Y-%m-%d")+1, xmax=as.Date(first(COVIDBYDAY$Date),"%Y-%m-%d") + lengthsim, 
  ...
  #           fill = "cornflowerblue", alpha = 0.1)+
    annotate("text", x = COVIDBYDAY[time1,]$Date -3, y = max(dataRt$upper,na.rm=TRUE)/2, label = "Social Distancing",angle=90,colour=wes_palette("Zissou1",n=5)[1],size=4)+
    annotate("text", x = COVIDBYDAY[time2,]$Date -2, y = max(dataRt$upper,na.rm=TRUE)/2, label = "Lockdown",angle=90,colour=wes_palette("Zissou1",n=5)[5],size=4)+
   # annotate("text", x = COVIDBYDAY[time3,]$Date +3, y = max(dataRt$upper,na.rm=TRUE)/2, label = "Lockdown",angle=90,colour=wes_palette("Zissou1",n=5)[5],size=4)+
   ...
   # geom_point(data=COVIDBYDAY[-(completeat:lastat),],aes(x=Date,y=CUMSUM,colour="Observed"), width=0.8,size=3)+#,colour="black")+
    ...
    ylab(label="")+
    ggtitle(expression("Time-varying estimate of R[t]"))+
    ...
    scale_colour_manual(values = c("black")) + scale_x_date(breaks = pretty_breaks(18))+ylim(0,3.3)+ geom_hline(yintercept=1, linetype="dashed", color = "black") #+
  #  annotate("text", x = lockdownrel +3, y = max(COVIDBYDAY$MAX,datanew.cases.entering.hospital.cumsum$upper,na.rm=TRUE)/2, label = "Lockdown Released",angle=90,colour=wes_palette("Zissou1",n=5)[5],size=4)
  
  Pacute24<- ggplot(data= datanewcases, aes(x=Date))+
 #   annotate(geom = "rect", xmin=as.Date(last(COVIDBYDAY$Date),"%Y-%m-%d")+1, xmax=as.Date(first(COVIDBYDAY$Date),"%Y-%m-%d") + lengthsim, 
  #           ymin=0, ymax=Inf,
  #           fill = "cornflowerblue", alpha = 0.1)+
    geom_point(data=COVIDBYDAY[-(completeat:lastat),],aes(x=Date,y=CASES,colour="Observed"), width=0.8,size=3)+#,colour="black")+
    ...
    ylab(label="")+
    ggtitle("Hospitalised in acute beds last 24h")+
    plotdet+theme(axis.title.x = element_blank(), axis.text.x = element_blank())+
    theme(
      legend.position = c(1, 0),
      legend.justification = c("right", "bottom"),
  ...
  #           fill = "cornflowerblue", alpha = 0.1)+
    ...
    annotate("text", x = COVIDBYDAY[time2,]$Date -2, y = max(COVIDBYDAY$MAX,datanew.cases.entering.hospital.cumsum$upper,na.rm=TRUE)/2, label = "Lockdown",angle=90,colour=wes_palette("Zissou1",n=5)[5],size=4)+
  ...
  #           fill = "cornflowerblue", alpha = 0.1)+
    ...
    annotate("text", x = COVIDBYDAY[time2,]$Date -1, y = max(datanew.cases.entering.IC$upper,na.rm=TRUE)/2, label = "",angle=90,colour=wes_palette("Zissou1",n=5)[5],size=5)+
 ...
 #            fill = "cornflowerblue", alpha = 0.1)+
    ...
    annotate("text", x = COVIDBYDAY[time2,]$Date -1, y = max(datanewcases$upper,na.rm=TRUE)/2, label = "",angle=90,colour=wes_palette("Zissou1",n=5)[5],size=5)+
 ...
 #   annotate(geom = "rect", xmin=as.Date(last(COVIDBYDAY$Date),"%Y-%m-%d")+1, xmax=as.Date(first(COVIDBYDAY$Date),"%Y-%m-%d") + lengthsim, 
  ...
  #           fill = "cornflowerblue", alpha = 0.1)+
   ...
   # geom_point(data=DEATHSBYDAY[(completeat:lastat),],aes(y=CUMSUM), width=0.8,size=3,colour=cols[5],alpha=0.4)+
    ...
    plotdet + scale_x_date(breaks = pretty_breaks(18))#+
    #scale_y_continuous(labels=comma)#+ theme(axis.title.x = element_blank(), axis.text.x = element_blank())
  ...
  #           fill = "cornflowerblue", alpha = 0.1)+
    ...
    scale_y_continuous("",sec.axis = sec_axis(~ . /sum(AGEPOP)*100 , name='%'))
...
#print(P8)
  ...
  #ALL 
#  grid.arrange(rbind(ggplotGrob(PRt),ggplotGrob(P1), ggplotGrob(P6), ggplotGrob(P8), ggplotGrob(P10),ggplotGrob(P7), size = "last"),
...
#  return(datanew.cases.entering.IC) #for peak of IC in 24hrs
   ...
   #print(dataSEROPREVsum)
   
  print(PRt)
  #grid.arrange(rbind(ggplotGrob(PRt),ggplotGrob(P5), ggplotGrob(P12), ggplotGrob(P4),size = "last"),ncol=2)
 
 #  grid.arrange(rbind(ggplotGrob(PRt),ggplotGrob(Pacute24), ggplotGrob(P6), size = "last"),
  #             rbind( ggplotGrob(P8), ggplotGrob(P10),ggplotGrob(P7), size = "last"),
   #            top = textGrob(TITLE,gp=gpar(fontsize=20,font=3)),ncol=2)
